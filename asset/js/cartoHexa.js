...
class cartoHexa {
    ...
    function svgBezierFusion(hS, hT){
        ...
        */
        ...
        //ATTENTION chaque hexa a ses propres bords
        //traite les points suivant les voisins
        neighborsIn = getNeighborsIn(hT.hex,hS.r.hexas);             
        neighborsIn.forEach(n=>{
            pHexa = n.layoutOut.polygonCorners(n.hex);
            switch (n.g) {
                case 'sw':
                    //change le voisin
                    if(n.pointsBezier.has('se0'))n.pointsBezier.set('se0',bp.se);
                    if(n.pointsBezier.has('ne0')){
                        n.pointsBezier.delete('ne0');
                        n.pointsBezier.set('ne_'+angleDir,[
                            bp.ne[3],                        
                            bp.ne[3],                        
                            [bp.se[0][0]-n.wBord/2,bp.ne[3][1]],
                            [bp.se[0][0]-n.wBord/2,bp.ne[3][1]],
                        ]);
                    }
                    //change la target
                ...
                case 'nw':
                    //change le voisin
                    k = getPointDir('se', n.pointsBezier);
                    n.pointsBezier.delete(k);
                    xRela = n.center.x-pHexa[2].x+n.wBord/2;
                    n.pointsBezier.set('se_'+angleDir,[
                        [xRela,bp.se[3][1]],
                        [xRela,bp.se[3][1]],
                        bp.se[3],                        
                        bp.se[3]                        
                    ]);    
                    k = getPointDir('ne', n.pointsBezier);
                    if(k=='ne0')n.pointsBezier.set(k,bp.ne);
                    //change la target
                    pHexa = hT.layoutOut.polygonCorners(hT.hex);
                    k = getPointDir('nw', hT.pointsBezier);
                    hT.pointsBezier.delete(k);
                    xRela = hT.center.x-pHexa[5].x-hT.wBord/2;
                ...
                case 'n':
                    ...
                    k = getPointDir('sw', n.pointsBezier);
                    n.pointsBezier.delete(k);
                    k = getPointDir('se', n.pointsBezier);
                    n.pointsBezier.delete(k);                                        
                    yRela = (n.center.y-pHexa[2].y-bp.se[0][1])/2;
                    n.pointsBezier.set('se_'+angleDir,[
                        [bp.se[0][0],bp.se[0][1]],
                        [bp.se[0][0],yRela],
                        [n.center.x-pHexa[2].x,yRela],
                        [n.center.x-pHexa[2].x-n.wBord/2,n.center.y-pHexa[2].y-n.wBord/2]
                    ]);
                    //modification de la target
                    k = getPointDir('nw', hT.pointsBezier);
                    hT.pointsBezier.delete(k);
                    k = getPointDir('ne', hT.pointsBezier);
                    hT.pointsBezier.delete(k);
                    pHexa = hT.layoutOut.polygonCorners(hT.hex);
                    yRela = (hT.center.y-pHexa[4].y-bp.nw[0][1])/2;
                    xRela = hT.center.x-pHexa[4].x; 
                    hT.pointsBezier.set('ne_'+angleDir,[
                        [bp.se[0][0],bp.nw[0][1]],
                        [bp.se[0][0],yRela],
                        [xRela,yRela],
                        [xRela-hT.wBord/2,hT.center.y-pHexa[4].y+hT.wBord/2]
                    ]);
                    break;
            }
